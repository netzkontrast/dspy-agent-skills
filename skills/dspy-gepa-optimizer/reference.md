...
Source: https://dspy.ai/api/optimizers/GEPA/overview/ (DSPy 3.3.1 surface verified September 2026).
...
## `dspy.BetterTogether` in 3.3.x

DSPy 3.2.0 expanded `dspy.BetterTogether` from a fixed prompt/weight pair into an arbitrary optimizer-chain wrapper, and 3.3.x keeps that shape:
...
The strategy keys come directly from the constructor kwargs (`bootstrap`, `gepa`, etc.), and `BetterTogether` evaluates each stage on the valset before returning the best program. Pass `strategy=` explicitly for named stages: the 3.3.1 default remains `"p -> w -> p"`, which assumes your optimizer keys are literally `p` and `w`.
...
- `dspy.BetterTogether` — since DSPy 3.2.0, chains arbitrary named optimizers via `**optimizers` and a strategy string.
