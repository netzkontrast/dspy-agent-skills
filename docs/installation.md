...
- **For running the end-to-end examples under `examples/`**: Python 3.10+, `uv` or `pip`, DSPy `3.3.1` for current local runs, `python-dotenv`, task-specific extras like `rank-bm25`/`pydantic`, and an `OPENROUTER_API_KEY` loaded from `.env`. Historical committed artifacts stay labeled with the DSPy version that produced them.
...
## Reproducing the tested DSPy 3.3.1 environment

The official DSPy install remains:

```bash
python -m pip install "dspy==3.3.1"
```

If you prefer `uv`, the exact command path we validated for this repo's example runs is:

```bash
env -u UV_EXCLUDE_NEWER uv run --with dspy==3.3.1 --with python-dotenv python -c 'import dspy; print(dspy.__version__)'
```
...
- or fall back to `python -m pip install "dspy==3.3.1"` in a local virtualenv
